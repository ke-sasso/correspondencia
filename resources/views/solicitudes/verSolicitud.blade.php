...
@if($info->idClasificacion==1)
    @if(!empty($estadoPart)) @if($estadoPart[0]==1)
            @include('solicitudes.paneles.accionesSolicitud')
    @endif @endif
 ...
 @if(in_array(477, $permisos, true))
  ...
  <div class="col-sm-12">
              <div class="panel panel-success">
                ...
                <div class="panel-body">
                     @include('solicitudes.paneles.verComentarios')
                </div><!-- /.panel-body -->
                <div class="panel-footer"></div>
              </div><!-- /.panel panel-success -->
  </div><!-- /.col-sm-4 -->
  ...
  @elseif(in_array(470, $permisos, true))
...
<!--
  <div class="col-sm-12">
              <div class="panel panel-success">
                ...
                <div class="panel-body">
                     @include('solicitudes.paneles.verObservacion')
                </div>
                <div class="panel-footer"></div>
              </div>
  </div><!-- /.col-sm-4 -->
 ...
 <div class="col-sm-12">
  <div class="table-responsive">
                          <table class="table table-th-block table-success table-hover" id="dt-archivo-final" style="font-size:14px;" width="100%">
                         <thead class="the-box dark full">
                             <tr>
                              <th>Documento final usuario</th>
...
@include('solicitudes.paneles.reportarSolicitud')
@include('solicitudes.paneles.justificacionProrroga')
@include('solicitudes.paneles.listaJustificacionesPro')
@endsection
...
function accionSol(id){
    //1.Reportar asignación 2.Prórroga de solicitud 3.Justificaciones de prórroga
    if(id==1){
       $('#formReportarSol').modal('toggle');
    }else if(id==2){
        $('#formJustificaciones').modal('toggle');
    }else if(id==3){
        var table = $('#dt-justificacion-pro').DataTable();
        table.destroy();
         $('#dt-justificacion-pro').DataTable({
        filter:true,
        processing: true,
        ajax: {
            url: "{{ route('get.historial.prorroga.sol') }}",
             data: function (d) {
                d.idSolicitud= {{ $info->idSolicitud}};
            }
        },
         columns: [
          {data: 'justificacion', name: 'justificacion'},
          {data: 'dias', name: 'dias'},
          {data: 'usuarioCreacion', name: 'usuarioCreacion'},
        ...
        "order": [[ 3, 'desc' ]]
          ...
          $('#formProrrogaHistorial').modal('toggle');
    }
}

$('#reportarSolasig').submit(function(e){
        var formObj = $(this);
        var formURL = formObj.attr("action");
      var formData = new FormData(this);
    $.ajax({
