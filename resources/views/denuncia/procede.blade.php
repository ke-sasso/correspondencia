...
@endif
		...
		<form  role="form" id="infoGeneral" method="post"  action="{{route('guardar.asistencia.denuncia')}}" autocomplete="off" enctype="multipart/form-data">
        ...
        <input type="hidden" name="tipoProcede" value="{{$tipoProcede}}" />
          
        <button type="submit" class="btn btn-primary btn-perspective">ENVIAR<i class="fa fa-check"></i></button>

        @if($formulario==1)
         <a href="{{route('ver.denuncia',['idSolicitud'=>Crypt::encrypt($idSolicitud)])}}" style="" type="button" id="cancelar" class="btn btn-warning btn-perspective">Regresar<i class="fa fa-reply" aria-hidden="true"></i></a>
        @else
          <a href="{{route('ver.denuncia.ciudadana',['idSolicitud'=>Crypt::encrypt($idSolicitud)])}}" style="" type="button" id="cancelar" class="btn btn-warning btn-perspective">Regresar<i class="fa fa-reply" aria-hidden="true"></i></a>
      ...
      success:  function (response){
            ...
            if(isJson(response)){
              alertify.alert("Mensaje de Sistema","<strong><p class='text-justify'>¡La información se ingreso con exito!</p></strong>",function(){
                ...
                // location.reload();
                window.location = "{{redirect()->getUrlGenerator()->previous()}}";  
